...
if (!function_exists('fireBase')) {
    function fireBase($user_id,$provider_id, $message,$type,$candle_number = null,$day = null)
    {
...
//        return $data;
        ...
        if($user_id != null)
            $tokens = \App\Models\Token::where('user_id', $user_id)->pluck('phone_token')->toArray();
        elseif($provider_id != null)
            $tokens = \App\Models\Token::where('provider_id', $provider_id)->pluck('phone_token')->toArray();
...
//        return $fields;
        ...
        $headers = array(
            'Authorization: key=' . "AAAAgZMIqcA:APA91bGfbNNNlG6S8VijF_9DlXZx871Y_Zbb_uZmiZBxJsmeAKx3rR6JaBWPPNS93wnleTWwmyEaSmjiJpFSDNBm8rJGIT-GVxdOcWcNbr88U5Y2S_7zNlYk7F_zudKhHDLsbwsNkJ_i",
...
if (!function_exists('userNotify')) {
    function userNotify($user_id,$provider_id,$order_id,$message,$type = 0)
    {
        $data = [
            'user_id'       => $user_id,
            'title'         => '',
...
//        return $data;
        ...
        $tokens = \App\Models\Token::where('user_id', $user_id)
            ->orWhere('provider_id',$provider_id)
            ->pluck('phone_token')
            ->toArray();

        if($provider_id != null){
            $tokens = \App\Models\Token::where('provider_id',$provider_id)
                ->pluck('phone_token')
                ->toArray();
        }
        elseif($user_id != null){
...
if (!function_exists('ArabicDate')) {
    function ArabicDate($date) {
        ...
        $ar_day_format_not_current = date("D", strtotime($your_date));
        $ar_day_format = date('D', strtotime($your_date)); // The Current Day
        $ar_day = str_replace($find, $replace, $ar_day_format);
        $ar_day_not_current = str_replace($find, $replace, $ar_day_format_not_current);
        ...
        return [
            'month'=>$ar_month,
            'ar_day'=>$ar_day,
            'ar_day_not_current' => $ar_day_not_current,
            'day'=>date('d'),
        ];
    }
}
